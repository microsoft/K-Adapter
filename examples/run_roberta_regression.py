...
class PretrainedModel(nn.Module):
    ...
    def forward(
        ...
        # last_hidden_cls = outputs[0][0, 0, :]
        # last_hidden_squeezed = last_hidden.squeeze(dim=1)

        logits = self.out_proj(self.dropout(last_hidden_cls))
...
#         fac_features_1 = fac_features[:, 0, :]
#         fac_features_squeezed = fac_features_1.squeeze(dim=1)
#         logits = self.out_proj(self.dropout(self.dense(fac_features_squeezed)))
...
def train(args, train_dataset, model, tokenizer):
    ...
    #         {'params': [p for n, p in regressor_model.named_parameters() if not any(nd in n for nd in no_decay)], 'weight_decay': args.weight_decay},
    #         {'params': [p for n, p in regressor_model.named_parameters() if any(nd in n for nd in no_decay)], 'weight_decay': 0.0}
    #         ]
    # else:
    #     optimizer_grouped_parameters = [
    #         {'params': [p for n, p in regressor_model.named_parameters() if not any(nd in n for nd in no_decay)],
    #          'weight_decay': args.weight_decay},
    #         {'params': [p for n, p in regressor_model.named_parameters() if any(nd in n for nd in no_decay)],
    #          'weight_decay': 0.0},
    ...
    optimizer = AdamW(
        pretrained_model.parameters(), lr=args.learning_rate, eps=args.adam_epsilon
    )
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            #     torch.nn.utils.clip_grad_norm_(regressor_model.parameters(), args.max_grad_norm)
...
def main():
    ...
    parser.add_argument(
        "--model_type", default=None, type=str, required=True, help="Model type"
    )
    parser.add_argument(
        "--model_name_or_path",
        default=None,
    ...
    tokenizer = RobertaTokenizer.from_pretrained(
        "roberta-large", model_max_length=args.max_seq_length
    )
    plain_bert = PretrainedModel(args)

    if args.meta_bertmodel:
        model_dict = plain_bert.state_dict()
        bert_meta_dict = torch.load(
            args.meta_bertmodel, map_location=lambda storage, loc: storage
        )
        for item in [
            "out_proj.weight",
            "out_proj.bias",
        ...
        changed_bert_meta = {
            k: v for k, v in changed_bert_meta.items() if k in model_dict.keys()
        }
        # print(changed_bert_meta.keys())
        model_dict.update(changed_bert_meta)
        plain_bert.load_state_dict(model_dict)
    if args.local_rank == 0:
        torch.distributed.barrier()  # Make sure only the first process in distributed training will download model & vocab

    plain_bert.to(args.device)
